interface UBrowser {
    ...
    }, savePath?: string): this;

    /**
     * 模拟设备
     */
    device(arg: ('iPhone 11' | 'iPhone X' | 'iPad' | 'iPhone 6/7/8 Plus' | 'iPhone 6/7/8' | 'iPhone 5/SE' | 'HUAWEI Mate10' | 'HUAWEI Mate20' | 'HUAWEI Mate30' | 'HUAWEI Mate30 Pro') | {
...
interface FeatureActionPayloadFile {
...
interface FeatureActionPayloadWindow {
    app: string;
    appPath: string;
    class: string;
    height: number;
...
interface MainPushOption {
    icon?: string,
    text: string,
    title?: string
}

interface FeatureActionByText {
    code: string;

    type: 'regex' | 'over' | 'text';
     ...
     */
    option?: MainPushOption;

}

interface FeatureActionByFile {
    code: string;

    type: 'files' | 'img';

    payload: Array<FeatureActionPayloadFile>

}

interface FeatureActionByWindow {
    code: string;

    type: 'window';

    payload: FeatureActionPayloadWindow

}

type FeatureAction = FeatureActionByText | FeatureActionByFile | FeatureActionByWindow;
...
interface FeatureCmdWindowMatch {
    // 应用（必须）
    app: string[];
    // 窗口标题正则 (可选)
    title?: string;
...
interface FeatureCmdWindow {
    // 类型标记（必须）
    type: 'window',
    // 关键字名称（必须）
    label: string,
    match: FeatureCmdWindowMatch;
}


type FeatureCmd = FeatureCmdRegex | FeatureCmdOver | FeatureCmdImg | FeatureCmdFile | FeatureCmdWindow | string;
...
interface UToolsApi {
     ...
     * 插件应用进入时触发
     */
    onPluginEnter(callback: (action: FeatureAction) => void): void;

    /**
     * 向搜索面板推送消息
     */
    onMainPush(callback: (action: FeatureAction) => {
        icon?: string,
        text: string,
        title?: string
    }[], selectCallback: (action: FeatureActionByText) => void): void;
     ...
     */
    createBrowserWindow(url: string, options: { width?: number, height?: number }, callback?: () => void): {
        id: number,
        [key: string]: any,
     ...
     */
    getFeatures(codes?: string[]): Array<Feature>;
     ...
     *
     * @param body 内容
     * @param clickFeatureCode plugin.json 配置的 feature.code，点击通知进入插件应用功能(该 feature.cmds 至少包含一个搜索字符串关键字)
     */
